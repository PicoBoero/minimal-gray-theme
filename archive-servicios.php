...
<main class="flex-grow flex flex-col justify-center pt-24 md:pt-40 container mx-auto px-4 py-16 md:py-32">
    <h1 class="text-3xl md:text-4xl font-bold text-gray-900 mb-8">Servicios</h1>
                ...
                <article class="card-premium border border-gray-100 shadow-premium w-full">
                    ...
                    <div class="p-8">
                        <h2 class="text-2xl font-bold text-gray-900 mb-3 tracking-tight">
                            <a href="<?php the_permalink(); ?>" class="hover:text-amber-500 transition-colors font-bold"><?php the_title(); ?></a>
                        </h2>
                        <p class="text-gray-400 text-xs font-bold uppercase tracking-widest mb-4">Servicio Profesional</p>
                        <p class="text-gray-600 leading-relaxed line-clamp-3"><?php echo wp_trim_words(get_the_excerpt(), 20, '...'); ?></p>
                        <a href="<?php the_permalink(); ?>" class="inline-flex items-center mt-6 text-xs font-black uppercase tracking-widest text-gray-900 group-hover:text-amber-600 transition-colors">Saber más <span class="ml-2 transform group-hover:translate-x-1 transition-transform">→</span></a>
                    </div>
                </article>
            <?php endwhile; ?>
        </div>
    ...
    <?php else : ?>
        <p class="text-gray-600 text-center">No hay servicios disponibles.</p>
    <?php endif; ?>
</main>
